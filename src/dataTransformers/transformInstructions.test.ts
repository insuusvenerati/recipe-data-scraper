  ...
  describe("expected behavior when passed an array of objects with another format", () => {
    ...
    it("logger should be invoked", () => {
      sinon.assert.calledOnceWithExactly(
        loggerStub,
        "recipe instructions array has different format",
        [{ random: "log-me" }]
      );
    });
  });
...
describe("transformInstructions no stubs", () => {
  ...
  before(() => {
    transformInstructions = proxyquire.noCallThru().load("./transformInstructions", {}).default;
  });
