...
describe("transformIngredients functionality", () => {
  ...
  describe("expected behavior when passed an array of strings", () => {
    ...
    before(() => {
      ...
      result = transformIngredients(["1 cup pasta", "three teaspoons vanilla", "1/4 tsp salt"]);
    });
    ...
    it("the argument should be returned as is", () => {
      result.should.eql([
        "cleaned 1 cup pasta",
        "cleaned three teaspoons vanilla",
        "cleaned 1/4 tsp salt",
      ]);
    });
  });
...
describe.skip("transformIngredients end to end", () => {
  ...
  before(() => {
    transformIngredients = proxyquire.noCallThru().load("./transformIngredients", {}).default;
  });
  ...
  describe("smittenkitchen microdata", () => {
    ...
    before(() => {
      result = transformIngredients(
        microdataUrls.smittenkitchen.fullResponse.items[0].properties.recipeIngredient
      );
    });
